...
/**
 * Assigns each data point in [mStart, mEnd) to its closest cluster centroid.
 * Each thread owns a disjoint range of points, so writes to
 * clusterAssignments[] never overlap -> no synchronization required.
 */
 ...
 *
 * This is the dominant phase (~68% of runtime in profiling), so it is the one
 * function we parallelize. We spread the M data points across worker threads;
 * because point m is only ever touched by one thread, there are no data races
 * and no locks. (Swapping the k/m loop nesting to m-outer also lets each point
 * keep its running minimum in a register instead of an M-sized scratch array.)
 */
void computeAssignments(WorkerArgs *const args) {
  const int numThreads = 8;
  std::thread workers[numThreads];
  int M = args->M;
  int chunk = (M + numThreads - 1) / numThreads;

  for (int t = 1; t < numThreads; t++) {
    int s = t * chunk, e = std::min(M, s + chunk);
    if (s < e) workers[t] = std::thread(assignPointsRange, args, s, e);
  }
  assignPointsRange(args, 0, std::min(M, chunk));   // main thread does chunk 0
